...
class combustivelBOA{
    ...
    public function ListaCombustivel($mbDados){
        ...
        if(isset($mbDados["id"]) && !empty($mbDados["id"]) ){
            $loId = $mbDados["id"];
            $loDdConsulta .=  " AND id_combustivel  = ".$loId; 
        ...
        /*if($_SESSION["supervisor"] != 1){    
            $loDdConsulta .= " AND combustivel.id_pessoa = ".$_SESSION["id_pessoa_matriz"];
        } */
    ...
    public function DesativarCombustivel($loDados){
        ...
        if(count($loDados) > 0 ){
            foreach ($loDados as $item){
                ...
                $loSql = "UPDATE combustivel SET ativo = 0 WHERE id_combustivel = ".$item;
                $query= $pdo->prepare($loSql);
                $query->execute();
            }
        }
