...
class pessoaBO{
    ...
    public function ListaPessoa($loDados){
        ...
        if( isset($loDados["motorista_condutor"]) && !empty($loDados["motorista_condutor"]) ){
            $loDdConsulta .=  " AND (pessoa.ind_motorista = 1 OR pessoa.ind_condutor = 1) "; 
        }

        if(isset($loDados["pesquisa_gestor"]) && !empty($loDados["pesquisa_gestor"])  ){
        ...
        if($_SESSION["supervisor"] != 1){    
         ...
         $loDdConsulta .= " AND tipo_pessoa.id_tipo_pessoa NOT IN(7)"; 
    ...
    public function GravarMotoristaPassageiro($loDados){
        ...
        if($loDados["cep"] == ""){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o Cep.";
        }
         if($loDados["endereco"] == ""){ 
            $loResultadoOperacao = true;
        ...
        if($loDados["bairro"] == ""){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o Bairro.";
        }   
        if($loDados["numero"] == ""){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o n&uacute;mero.";
        }   
        if($loDados["id_estado"] == ""){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o Estado.";
        } 
        if($loDados["id_cidade"] == ""){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o Cidade.";
        }                                
        if($loDados["email"] == ""){ 
            $loResultadoOperacao = true;
        ...
        if($loDados["num_habilitacao"] == "" && ($loDados["ind_motorista"] == "1" || $loDados["ind_condutor"] == "1") && $loValidacaoTela == "pessoa"){ 
            $loResultadoOperacao = true;
            $loResultadoMessagem = "Favor preencher o n&uacute;mero habilita&ccedil;&atilde;o.";
        }
        if($loDados["data_validade_habilitacao"] == "" && ($loDados["ind_motorista"] == "1" || $loDados["ind_condutor"] == "1") && $loValidacaoTela == "pessoa"){ 
            $loResultadoOperacao = true;
        ...
        // Verifica se login ja esta cadatrado se false cadastrado se true liberado 
        if( isset($loDados["login"]) && !empty($loDados["login"]) && $loDados["acao"] == "I" ){
            $loValidaLoginExiste = $this->ValidaLogin($loDados);
            if(!$loValidaLoginExiste){
                $loResultadoOperacao = true;
                $loResultadoMessagem = "Login j&aacute; cadastrado!";
            }
        }
        ...
        if(isset($loDados["logoArquivo"]["arquivo"]) && $loDados["logoArquivo"]["arquivo"]["name"] != "" ){
            ...
            if(isset($loDados["validacao"]) && $loDados["validacao"] == 0)
            {
               ...
               if($loTypeArquivo == "image/jpeg" || $loTypeArquivo == "image/png")
               {
                    ...
                    $rand = rand(2, 30);
                    $nome_empresa = str_replace("/", "", $nome_empresa);
                    $loNomeArquivo = $nome_empresa."_logo_".$rand.".png";
   ...
   public function ListaMotoristaVencHabilitacao($loDados){
        ...
        if( isset($loDados["ind_gestor"]) && !empty($loDados["ind_gestor"]) ){
            $loDdConsulta .=  " AND id_pessoa IN(
                                    SELECT autorizadores.id_pessoa_filho FROM pessoa
         ...
         if( isset($loDados["ind_usuario"]) && !empty($loDados["ind_usuario"]) ){
             $loDdConsulta .=  " AND id_pessoa IN( SELECT id_pessoa_origem FROM usuario WHERE id_usuario = ".$_SESSION["id_usuario"]." ) ";
         }
        ...
        $loDdConsulta .=  " ORDER BY DATEDIFF(NOW(),pessoa.validade_habilitacao) DESC ";
    ...
    public function ValidaLogin($loDados){
        
        $loPessoa = new pessoaBOA();
        $loRetorno = $loPessoa->ValidaLogin($loDados);
        return $loRetorno;    
    }

    public function RemoverImagemLogo($loDados){
        
        $loPessoa = new pessoaBOA();
        $loRetorno = $loPessoa->RemoverImagemLogo($loDados);
        return $loRetorno;    
    }

    public function InserirCamposTabelasDinamica($loDados){

        $loPessoa = new pessoaBOA();
        $loRetorno = $loPessoa->InserirCamposTabelasDinamica($loDados);
        return $loRetorno;    
