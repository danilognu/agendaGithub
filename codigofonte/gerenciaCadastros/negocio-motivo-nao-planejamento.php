<?php
include('persistencia-motivo-nao-planejamento.php');

class motivoNaoPlanejamentoBO{
    ...
    public function Consultar($loDados){
        ...
        if(isset($loDados["id"]) && !empty($loDados["id"]) ){
            $loId = $loDados["id"];
            $loDdConsulta .=  " AND id_mot_plan = ".$loId; 
        ...
        $loConsultar = new motivoNaoPlanejamentoBOA();
    ...
    public function Gravar($loDados){
        ...
        if($loResultadoOperacao){
            ...
            $loItens = new motivoNaoPlanejamentoBOA();
